...
- [ ] **Step 2: scope ゲーティングを切り分け検証（local と project の両方）**

**マーカーを差別化要因から外す**ため、全テストプロジェクトに `mkdir -p .claude && touch .claude/codex-design-review.enabled` を作成しておく。発火差は scope 由来であることを保証する。

*(a) local スコープ*
1. project A に現行プラグインを **local スコープ**で導入（`/plugin install codex-design-review@<marketplace> --scope local` を A の中で、または `/plugin` UI で Local）。project B には導入しない。
2. A で `docs/superpowers/specs/verify.md` に実質的内容を書く → review skill 起動（additionalContext 注入）を**観察**。
3. B で同じ書き込み → **何も起きない**ことを観察。

*(b) project スコープ*
4. 別の project C に **project スコープ**で導入（`--scope project`、または UI の Project。`.claude/settings.json` に書かれコミット対象）。project D には導入しない。
5. C で spec 書き込み → **発火**を観察。D で同じ → **非発火**を観察。

- [ ] **Step 3: 合否判定とブランチ選択**

- **local（A 発火 / B 非発火）かつ project（C 発火 / D 非発火）の両方が成立** → scope ゲーティング成立。**Task 3・4・8 を「通常版（フラグ廃止）」で実施**。
- **どちらか一方でも不成立** → その scope は README の導入導線から外す。両方不成立なら **Task 3・4・8 を「フォールバック版（自己ゲート維持）」で実施**（各タスクの「フォールバック」節を参照）。
...
Task 1 preflight で得た結論を、英語化・フラグ廃止後の実体で最終確認: 改修後プラグインを **local スコープ**で project A に再導入し A で発火・B 非発火、**project スコープ**で project C に再導入し C で発火・D 非発火、をそれぞれ再観察（発火時の additionalContext が `/codex-design-review:review` を指すこと）。Task 1 の記録ファイルに「最終確認: 合格（local/project 両方）」を追記。
