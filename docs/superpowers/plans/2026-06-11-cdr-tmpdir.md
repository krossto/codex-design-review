...
- [ ] **Step 1: 正常系（TMPDIR 未設定）— `/tmp` 経路で WORK= が返ることを確認**

検証対象スニペット（このまま実行）。`unset TMPDIR` を明記し、`${TMPDIR:-/tmp}` が `/tmp` に解決されることを確かめる:
...
- [ ] **Step 2: 正常系（TMPDIR 設定済み）— TMPDIR が尊重されることを確認**

spec の「TMPDIR 尊重」を実証する。一時的な root を作って `TMPDIR` に渡す:
...
work=""
work="$(mktemp -d "${TMPDIR:-/tmp}/codex-design-review-XXXXXX" 2>/dev/null)" || true
[ -n "$work" ] || { echo "ERROR"; exit 1; }
echo "WORK=$work"
case "$work" in "$tmp_root"/codex-design-review-*) echo "TMPDIR-respected" ;; *) echo "TMPDIR-IGNORED" ;; esac
rm -rf -- "$tmp_root"
'
```

Expected: `WORK=<tmp_root>/codex-design-review-XXXXXX` と `TMPDIR-respected` が出力される（`TMPDIR-IGNORED` が出たら実装が `TMPDIR` を無視している）。

- [ ] **Step 3: フォールバック系 — TMPDIR が書けないとき .claude/tmp-cdr へ落ちる**
...
- [ ] **Step 4: 両方失敗系 — ERROR= と exit 1 を確認**

`exit 1` はスニペットを終了させるため、終了コードはサブシェルの外側で受け取って確認する:
...
rc=$?
echo "rc=$rc"
test "$rc" -eq 1 && echo "EXIT-OK"
```

Expected: `ERROR=could not create codex-design-review work dir` が出力され（`WORK=` は出ない）、続いて `rc=1` と `EXIT-OK` が出力される。

- [ ] **Step 5: 後始末検証 — 残骸が残らず、空 `$work` では rm が走らないこと（spec §6.3）**

通常後始末（ディレクトリが消えること）:

```bash
bash -c '
work="$(mktemp -d "${TMPDIR:-/tmp}/codex-design-review-XXXXXX")"
touch "$work/probe"
[ -n "${work:-}" ] && rm -rf -- "$work"
test ! -e "$work" && echo "CLEANUP-OK"
'
```

Expected: `CLEANUP-OK`。

空ガード（`$work` が空のとき `rm` を呼ばないこと）:

```bash
bash -c '
rm() { echo "rm-called"; return 99; }   # rm が呼ばれたら検知
work=""
[ -n "${work:-}" ] && rm -rf -- "$work"
echo "ok"
'
...
Expected: `ok` のみ（`rm-called` が出ない＝空 `$work` では `rm` が起動しない）。

> 注: SKILL.md 本体では fallback の `mkdir -p` に `2>/dev/null` は付けない（通常はエラーを見せたい）。両方失敗系の検証用に Step 4 だけ `2>/dev/null` を付けてノイズを抑えている。本体スニペットは Step 1 のものを正とする。
...
`docs/superpowers/specs/2026-06-11-cdr-tmpdir-design.md` の §6（1〜4）を読み、各項目が以下で実証済みであることを確認する:
- §6.1 既存 bats グリーン → Task 5 Step 1
- §6.2 正常系（TMPDIR 未設定/設定済み）・フォールバック系・両方失敗系 → Task 1 Step 1〜4
- §6.3 後始末（残骸が残らない／空 `$work` で rm 不起動）→ Task 1 Step 5
- §6.4 README から uuidgen 削除 → Task 4 Step 2 / Task 5 Step 3

全項目に対応ステップがあること。万一未対応が見つかったらこの場で対応ステップを補う。

---

## Self-Review メモ

- **Spec coverage:** §3.2 解決→Task1/2、§3.3 固定名パス→Task2/3、§5 両方失敗スキップ＋空ガード cleanup→Task2/3、§2.6 README→Task4、§6 検証→Task1（Step1〜5: TMPDIR 未設定/設定済み・フォールバック・両方失敗・後始末）/Task5。全要件にタスク対応あり。
